...
@router.post("/bonuses/import-commission", response_model=BonusResponse)
async def import_commission_bonus(
    ...
    if not (user.is_admin or user.is_dg or user.is_drh or user.is_validator_n1):
        raise HTTPException(status_code=403, detail="Vous n'avez pas le droit de créer des commissions")

    employee = await Employee.get(id=employee_id)
    content = await file.read()
    # Essayer d'abord en tant que Excel (.xlsx), sinon CSV
    try:
    ...
    # En-têtes attendus: designation, ventes, objectif
    # Ou: product, ventes, objectif
    header_row = rows[0] if rows else []
    col_map = {}
    for i, header in enumerate(header_row):
        h = str(header).strip().lower()
        if h in ('designation', 'product', 'nom', 'libelle', 'name'):
            col_map['designation'] = i
        if h in ('ventes', 'quantite', 'quantite_vendu', 'ventes_realisées'):
            col_map['ventes'] = i
        if h in ('objectif', 'target', 'but', 'objectif_ventes'):
            col_map['objectif'] = i

    # Taux de commission (viendra du formulaire, on utilise une valeur par défaut)
    # Dans un premier temps, on utilisera le taux par défaut 0 et le calculera
    # Pour l'instant, on stocke les données brutes

    sales = []
    total_commission = 0

    # Le taux de commission sera celui saisi dans le formulaire
    # Pour l'instant on utilise 0, l'utilisateur devra saisir le taux
    rate = 0  # Sera remplacé par le taux du formulaire

    # Mapping des colonnes selon ce qui est détecté
    for row in rows[1:]:  # Skip header
        designation = row[col_map.get('designation', 0)] if col_map.get('designation', 0) < len(row) else ''
        ventes_str = row[col_map.get('ventes', 1)] if col_map.get('ventes', 1) < len(row) else '0'
        objectif_str = row[col_map.get('objectif', 2)] if col_map.get('objectif', 2) < len(row) else '0'

        try:
            ventes = float(ventes_str)
        ...
        # Calcul de la commission selon la règle:
        # ventes >= objectif → (ventes × taux) × 2
        # ventes < objectif → ventes × taux
        if objectif > 0 and ventes >= objectif:
            commission = (ventes * rate) * 2 if rate else 0
        else:
            commission = ventes * rate if rate else 0

        total_commission += commission

        sales.append({
            'designation': str(designation) if designation else 'Produit',
            'ventes': ventes,
            'objectif': objectif,
    ...
    obj = await Bonus.create(
        employee_id=employee_id,
        start_date=start_date,
        end_date=end_date,
        bonus_type=BonusType.COMMISSION,
        taux_commission=rate,
